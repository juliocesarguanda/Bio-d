...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            return res.status(200).json({ estatus: 'exito', respuesta: 'Examen agregado correctamente' });
        } catch (error) {
        ...
        reportError(__filename, new Date(), error.message, req.originalUrl, req.body);
        return res.status(500).json({ estatus: 'error', respuesta: 'Error al agregar el examen: ' + error.message });
    }
});
