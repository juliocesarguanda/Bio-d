...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            res.status(200).json({ estatus: 'exito', respuesta: 'Exito al eliminar el examen' });
        } catch (error) {
        ...
        reportError(__filename, new Date(), error.message, req.originalUrl, req.body);
        res.status(500).json({ estatus: 'error', respuesta: 'Error al eliminar el examen: ' + error.message });
    }
});
