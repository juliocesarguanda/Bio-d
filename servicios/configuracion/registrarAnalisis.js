...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            res.status(201).json({ estatus: 'exito', respuesta: 'Análisis insertado correctamente' });
        } catch (error) {
        ...
        reportError(__filename, new Date(), error.message, req.originalUrl, req.body);
        res.status(500).json({ estatus: 'error', respuesta: 'Error al registrar el análisis: ' + error.message });
    }
});
