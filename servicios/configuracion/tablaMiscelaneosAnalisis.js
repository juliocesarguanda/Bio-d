...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const query = `
            ...
            FROM miscelaneo 
            WHERE estatus = 1 
              AND id NOT IN (SELECT miscelaneo FROM miscelaneo_analisis WHERE analisis = ?)
        `;
        ...
        if (resultados.estatus === 'error') {
            ...
            return res.status(500).json({ estatus: 'error', respuesta: 'Error al consultar los datos de misceláneos analisis' });
        }
