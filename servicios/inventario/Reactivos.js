...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (resultadosReactivos.estatus === 'error') {
            reportError(__filename, new Date(), resultadosReactivos.respuesta, req.originalUrl, {});
        ...
        const filename = `REACTIVOS ${moment().format('DD-MM-YYYY')}.pdf`;
        ...
        const drawHeader = (pageNumber) => {
            ...
            doc.fillColor('white')
                .text(`REACTIVOS ${formattedDate}`, 0, 0 + height / 2 + 8, {
                    width: width,
                    align: 'center'
                });
        ...
        resultadosReactivos.respuesta.map(r => {
            index++
            if (index > 35) {
                pag++
                doc.addPage();
            ...
            let text;
            text = doc.font('Helvetica').fillColor('black').fontSize(10).text(r.nombre, startX + cellPadding, startY + cellPadding, { width: 205 - cellPadding * 2, align: 'left' }).height;
            if (text > maxHeight) { maxHeight = text; }
            doc.rect(startX, startY, 205, maxHeight + cellPadding * 2).stroke();
            startX += 205;

            text = doc.font('Helvetica').fontSize(10).text(r.marca, startX + cellPadding, startY + cellPadding, { width: 205 - cellPadding * 2, align: 'left' }).height;
            if (text > maxHeight) { maxHeight = text; }
            doc.rect(startX, startY, 205, maxHeight + cellPadding * 2).stroke();
            startX += 205;

            text = doc.font('Helvetica').fontSize(10).text(r.cant, startX + cellPadding, startY + cellPadding, { width: 90 - cellPadding * 2, align: 'left' }).height;
            if (text > maxHeight) { maxHeight = text; }
